...
bool vkw::VulkanSwapchain::AcquireNextImage(VkSemaphore presentComplete)
{
	VkResult result = vkAcquireNextImageKHR(m_pDevice->GetDevice(), m_Swapchain, UINT64_MAX, presentComplete, VK_NULL_HANDLE, &m_ActiveSwapchainImageId);
...
bool vkw::VulkanSwapchain::PresentImage(VkSemaphore waitSemaphore)
{
	...
	VkResult result = vkQueuePresentKHR(m_pDevice->GetQueue(), &presentInfo);
...
void VulkanSwapchain::Init()
{
	...
	if (m_SwapchainImageCount < surfaceCapabilities.minImageCount) m_SwapchainImageCount = surfaceCapabilities.minImageCount;
...
void vkw::VulkanSwapchain::RecreateSwapchain()
{
	m_OldSwapchain = m_Swapchain;
	Init();
	vkDestroySwapchainKHR(m_pDevice->GetDevice(), m_OldSwapchain, nullptr);
