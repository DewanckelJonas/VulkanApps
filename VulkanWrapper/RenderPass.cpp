...
RenderPass::RenderPass(VulkanDevice* pDevice, const std::vector<VkAttachmentDescription>& attachments, std::vector<VkSubpassDescription> subpassDescriptions, std::vector<VkSubpassDependency> dependencies)
	:m_pDevice(pDevice), m_AttachmentDescriptions(attachments), m_SubPasses(subpassDescriptions), m_SubPassDependencies(dependencies)
{
	Init();
}
