...
{
	return !m_IsAppClosing && m_pWindow->Update();
}
...
void VulkanBaseApp::OnWindowResize(Window*)
{
	if (!m_IsInitialized)
		return;
	m_IsInitialized = false;

	while(m_pWindow->GetSurfaceSize().width <= 0.f && m_pWindow->GetSurfaceSize().height <= 0.f)
	{
		//Pauses the window on resize
		m_pWindow->WaitOnEvents();
	}
...
void vkw::VulkanBaseApp::AcquireNextImage(VkSemaphore semaphore)
{
	if(!m_pSwapchain->AcquireNextImage(semaphore))
	{
		OnWindowResize(m_pWindow);
	}
}

void vkw::VulkanBaseApp::PresentImage(VkSemaphore renderCompleteSemaphore)
{
	if(!m_pSwapchain->PresentImage(renderCompleteSemaphore))
	{
		OnWindowResize(m_pWindow);
	}
}

const std::string& VulkanBaseApp::GetName()
{
	return m_AppName;
}
...
void VulkanBaseApp::InitWindow(uint32_t width, uint32_t height)
{
	...
	m_pWindow->AddOnResizeCallback(std::bind(&VulkanBaseApp::OnWindowResize, this, std::placeholders::_1));
}
...
DepthStencilBuffer* vkw::VulkanBaseApp::GetDepthStencilBuffer()
{
